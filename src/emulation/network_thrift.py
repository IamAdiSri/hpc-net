...
from lib.fattree_thrift import FatTreeTopo
...
# initialize network
net = FatTreeTopo(k=K, p4src="switch.p4", drop_port=DROP_PORT, ctrl_port=CTRL_PORT, p4rt=True, loglevel="info")
...
for hname in net.ft_hosts:
    ...
    hobj.cmd(
        f"nohup python3 -u test.py {hobj.intf().name} > outputs/{hname}_s.txt &"
    )
